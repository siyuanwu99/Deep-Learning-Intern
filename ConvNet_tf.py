...
def cnn_modul(X_train,Y_train,X_test,Y_test,
            learning_rate = 0.09,num_epoches = 100, minibatch_size = 64):
    """
    ...
    W4 = tf.Variable(tf.random_normal([16,6]), name='W4', dtype=tf.float32)
    ...
    'Cost'
    cost_sum = tf.nn.softmax_cross_entropy_with_logits_v2(labels= Y, logits= A4)
    ...
    correct_prediction = tf.equal(tf.argmax(A4, 1), tf.argmax(Y, 1))
...
print(cnn_modul(X_train,Y_train,X_test,Y_test,0.09,100,64))
